package model.command.core.fdtw;
...
public class FDTW
{
	...
	//Otras variables
	public String FileQ, FileC;  //Nombres de los archivos para guardar las secuencias
	...
	private void BackTracking(){
		...
		for(int z = 0; z < Path.length -1; z++) 
			if(Math.abs(Path[z].q - Path[z + 1].q) == 1 && Math.abs(Path[z].c - Path[z + 1].c) == 1){ //Do something with subsequence 
				Path[z].IsSub = true; 
				Path[z].nsub = nsub; 
				Path[z + 1].IsSub = true; 
				Path[z + 1].nsub = nsub; 
	...
	public void WriteSubsequences(String folder_dir, boolean zeroAverage_oneSplit){ 
		try 
		{
			String dir; 
			if(zeroAverage_oneSplit) 
				//dir = folder_dir + "subs_"+ ExtractFileName(FileQ) + "_"+ ExtractFileName(FileC) + "\\"; 
				dir = folder_dir + "subs_"+ (FileQ) + "_"+ (FileC) + "\\";
			else
				dir = folder_dir + "AVsubs_"+ (FileQ) + "_"+ (FileC) + "\\"; 
				//dir = folder_dir + "AVsubs_"+ ExtractFileName(FileQ) + "_"+ ExtractFileName(FileC) + "\\";
			...
			*/
			...
			if(zeroAverage_oneSplit)
				for(int z = 0; z < Path.length; z++){
					...
					if(Path[z].IsSub){
						int sub = Path[z].nsub;
						//using(StreamWriter sq = new StreamWriter(dir + (sub+1) + "_q.txt")){
							File sq = new File(dir+(sub+1)+"_q.txt");
							FileWriter f1 = new FileWriter(sq, true);
							//using(StreamWriter sc = new StreamWriter(dir + (sub+1) + "_c.txt")){ 
				...
				for(int z = 0; z < Path.length; z++){ 
					...
					if(Path[z].IsSub){
						int sub = Path[z].nsub; 
						//using(StreamWriter sw = new StreamWriter(dir + (sub+1) + "_av.txt")){ 
